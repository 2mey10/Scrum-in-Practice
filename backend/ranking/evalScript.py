...
def evaluate_model(model_path, test_data_loader, task):
    ...
    if task == "Klassifizierung":
        ...
        precision_score_value = precision_score(y_true=target.detach().numpy(), y_pred=preds.detach().numpy(), average='weighted')
        ...
        recall_score_value = recall_score(y_true=target.detach().numpy(), y_pred=preds.detach().numpy(), average='weighted')
